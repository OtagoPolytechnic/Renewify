...
public class WirePlacement : MonoBehaviour
{
    ...
    {
        if (connectedBuildings.Contains(BuildingPlacing.GetTilePosition(gridPosition)))
        {
            return true;
        }
        else
    ...
    private void WirePlacingLogic()
    {
        ...
        if (Input.GetMouseButton(0) && MouseManager.isHovering)
        {
            ...
            if (GridManager.IsTileEmpty(BuildingPlacing.GetTileIndex(MouseManager.gridPosition)))
            {
                int rotation = 0;
                ...
                if (illegalMoveCheck())
                {
                    ...
                    {
                        RemoveWire(BuildingPlacing.GetTileIndex(tile));
                    }
                    resetTileList();
                    return;
                }
                ...
                PlaceWire(BuildingPlacing.GetTileIndex(MouseManager.gridPosition), MouseManager.Instance.playerX, MouseManager.Instance.playerZ, rotation, StraightWire);
            ...
            else if (GridManager.Instance.tileStates[BuildingPlacing.GetTileIndex(MouseManager.gridPosition)] == goal && !illegalMoveCheck())
            {
                //Checks if the last wire needs to become a corner wire
                PlacingCornerWire(MouseManager.gridPosition);
                //Can swap out the material based on the building when we add the materials
                switch (GridManager.Instance.tileStates[BuildingPlacing.GetTileIndex(startingTile)])
                {
            ...
            else if ((tilesPlaced.Contains(MouseManager.gridPosition) || startingTile == MouseManager.gridPosition) && lastTile != MouseManager.gridPosition)
            {
                ...
                while (tilesPlaced.Count > 0 && tilesPlaced[tilesPlaced.Count - 1] != MouseManager.gridPosition)
                {
                    ...
                    RemoveWire(BuildingPlacing.GetTileIndex(tilesPlaced[tilesPlaced.Count - 1]));
    ...
    private void EndWirePlace()
    {
        foreach (Vector2 tile in tilesPlaced)
        {
            changeColour(GridCreator.tiles[BuildingPlacing.GetTileIndex(tile)].transform.GetChild(0).gameObject, CompletedConnection);
        }
    ...
    private void PlacingCornerWire(Vector2 currentTile)
    {
            ...
            (lastTile.y != secondLastTile.y || lastTile.y != MouseManager.gridPosition.y))
        {
            ...
            bool isMovingRightOrUpSecondLastToLast = BuildingPlacing.GetTileIndex(secondLastTile) <  BuildingPlacing.GetTileIndex(lastTile);
            bool isMovingRightOrUpLastToCurrent =  BuildingPlacing.GetTileIndex(lastTile) <  BuildingPlacing.GetTileIndex(currentTile);
        ...
        {
            PlaceWire( BuildingPlacing.GetTileIndex(lastTile), (int) lastTile.x, (int) lastTile.y, rotation, lastWire);
        }
    }
    ...
    private bool illegalMoveCheck()
    {
            ...
            (Math.Abs(MouseManager.gridPosition.x - lastTile.x) > 0)) //If a diagonal movement is made
        {
            ...
            if (GridManager.IsTileEmpty(BuildingPlacing.GetTileIndex(square1)) || GridManager.IsTileEmpty(BuildingPlacing.GetTileIndex(square2)))
            {
                Vector2 emptySquare;
                // Place a wire on the empty square
                if (GridManager.IsTileEmpty(BuildingPlacing.GetTileIndex(square1)))
                {
                    emptySquare = square1;
                }
                else
                ...
                PlaceWire(BuildingPlacing.GetTileIndex(emptySquare), (int) emptySquare.x, (int) emptySquare.y, 0, StraightWire);
